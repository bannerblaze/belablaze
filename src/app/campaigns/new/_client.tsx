...
import {
  ...
  MapPin, AlignLeft, Building2, Plus, X,
} from "lucide-react";
...
const STEPS = ["Cliente", "Detalles", "Presupuesto & Fechas", "Confirmar"];
...
export function NewCampaignClient({ clients }: { clients: ClientRef[] }) {
  ...
  const { register, handleSubmit, watch, formState: { errors }, trigger } = useForm<FormValues>({
    defaultValues: { name: "", description: "", clientId: "", budget: undefined, startDate: "", endDate: "" },
  });
  ...
  const canGoNext = async () => {
    if (step === 0) return await trigger("clientId");
    if (step === 1) return await trigger(["name", "description"]);
    if (step === 2) return await trigger(["budget", "startDate", "endDate"]);
    return true;
  };
          ...
          >
                    ...
                    {clients.map((client) => (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className={cn("text-sm font-medium", values.clientId === client.id ? "text-[#B8EB23]" : "text-white")}>{client.name}</p>
                  ...
                  {values.budget > 0 && (
                    <div className="p-3 rounded-xl bg-[#B8EB23]/[0.04] border border-[#B8EB23]/15">
                      ...
                      {values.startDate && values.endDate ? (() => {
                        const days = Math.max(1, Math.ceil((new Date(values.endDate).getTime() - new Date(values.startDate).getTime()) / 86400000));
                        return (
            ...
            {step === 3 && (
              <Card>
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-9 h-9 rounded-xl bg-green-400/10 flex items-center justify-center text-green-400">
                  ...
                  <div className="p-3 rounded-xl bg-blue-400/[0.06] border border-blue-400/15">
                    <p className="text-xs text-blue-400">La campaña se creará en estado <strong>Borrador</strong>. Podrás activarla cuando los anuncios estén aprobados.</p>
                  </div>
                </CardContent>
              </Card>
            )}
