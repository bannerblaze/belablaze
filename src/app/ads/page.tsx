      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            >
              {[
                { icon: <Eye className="w-3.5 h-3.5" />, label: "Ver detalles", onClick: () => {} },
                { icon: ad.status === "PAUSED" ? <Play className="w-3.5 h-3.5" /> : <Pause className="w-3.5 h-3.5" />, label: ad.status === "PAUSED" ? "Activar" : "Pausar", onClick: () => {} },
                { icon: <Copy className="w-3.5 h-3.5" />, label: "Duplicar", onClick: () => {} },
                { icon: <QrCode className="w-3.5 h-3.5" />, label: "Ver QR", onClick: () => {}, show: ad.qrEnabled },
                { icon: <Trash2 className="w-3.5 h-3.5" />, label: "Eliminar", onClick: () => {}, danger: true },
              ]
...
export default function AdsPage() {
  ...
  const totalImpressions = mockAds.reduce((s, a) => s + a.impressions, 0);
  const totalClicks = mockAds.reduce((s, a) => s + a.clicks, 0);
  const avgCTR = totalImpressions > 0 ? (totalClicks / totalImpressions) * 100 : 0;
  const pendingCount = mockAds.filter(a => a.status === "PENDING_REVIEW").length;

  return (
    <div className="p-6 space-y-5 max-w-[1600px]">
      {/* Summary metrics */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricCard title="Total anuncios" value={mockAds.length} icon={<Zap className="w-5 h-5" />} index={0} />
